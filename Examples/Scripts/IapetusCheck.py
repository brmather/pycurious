...
#    continue
    ...
    # compute radial spectrum
#try:
S, k, dtheta = grid.azimuthal_spectrum(subgrid, taper=None, scale=0.001, theta=5.0)

# plot
#fig = plt.figure(figsize=(12,3))
#ax1 = fig.add_subplot(111, xlabel='wavenumber (rad/km)', ylabel='theta ($^{\circ}$)')
#im1 = ax1.imshow(S, extent=[k.min(), k.max(), dtheta.min(), dtheta.max()], aspect=0.025)
#fig.colorbar(im1, label='Azimuthal spectrum')
#plt.show()

for t in range(dtheta.size):
    Zb= grid.Tanakab(S[t,:], k, kmin=0.05, kmax=0.2)
    f.write(' '.join([str(dtheta[t]),str(Zb),"\n"]))
#print(Zb)
